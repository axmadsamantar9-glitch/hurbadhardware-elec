...
async function runDogfood(): Promise<number> {
  ...
  try {
    ...
    console.log(`[dogfood-hur51] PASS: Correlation ID present: ${healthResult.response?.correlationId}`)
    ...
    if (ids.size >= 2) {
      console.log(`[dogfood-hur51] PASS: Correlation IDs are unique across requests (${ids.size} unique IDs)`)
    } else {
    ...
    console.error(`[dogfood-hur51] FAIL: ${error instanceof Error ? error.message : String(error)}`)
    devServer.kill()
    return 1
  }
}
