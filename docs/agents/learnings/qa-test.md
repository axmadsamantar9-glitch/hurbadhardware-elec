...
## HUR-191: Checkout / Atomic Order Creation (HUB-38) — Independently Re-Running a Builder-Claimed Live-DB Concurrency Test (2026-08-31)

**Symptom:** Highest-stakes ticket this session — atomic order creation (stock decrement + coupon redemption + address validation in one transaction), a refactor of the already-verified `adjustStock()` (HUB-29), and a builder-claimed live-DB concurrency test proving no-oversell holds for checkout specifically (not just isolated inventory adjustment). Task explicitly called for re-running the concurrency test myself, not trusting the builder's report of what it showed.

**Verification method and findings:**

1. **Full suite**: `npm run test -- --run --coverage` → 774/774 passing, 69 test files, matching the builder's claim. Coverage 90.87% stmts / 83.44% branches / 91.7% funcs / 91.97% lines — all above 80/70/80/80.
2. **Live concurrency test** (`src/lib/api/checkout.live.test.ts`): read it end-to-end — it is real, not staged. It creates a throwaway Category/Product (`stockQuantity: 1`)/2 Users/2 Addresses/2 Carts (each with 1 unit of the same product) directly against the live dev Postgres via `db.*.create`, fires `Promise.all([placeOrder(userA,...), placeOrder(userB,...)])` (true concurrent dispatch, not sequential awaits), and asserts exactly one `ok:true`/one `ok:false` with `error:"insufficient_stock"`, final `stockQuantity === 0`, exactly one `Order` row created, winning cart cleared and losing cart left with its item (proving the loser's transaction rolled back cleanly, no partial writes). It has a proper `afterAll` cleanup deleting only its own throwaway rows, and self-skips via `describe.skip` when `DATABASE_URL` is unset (same precedent as HUB-29's `inventory.live.test.ts`). **Re-ran it in isolation myself twice** (`npx vitest run src/lib/api/checkout.live.test.ts`) — passed both times, ~13-14s each (real DB round-trips, not mocked). This independently confirms the builder's report rather than trusting it.
3. **`adjustStock()` refactor**: `git diff HEAD -- src/lib/inventory.ts` shows a pure extraction — the new `applyStockDelta(tx, params)` contains the exact same SQL text (`UPDATE products SET stock_quantity = stock_quantity + ${delta} WHERE id = ${productId} AND stock_quantity + ${delta} >= 0`, and the variant equivalent) that previously lived inline in `adjustStock()`, byte-for-byte, just moved. `adjustStock()` now calls `applyStockDelta(tx, ...)` and keeps its own `db.$transaction` + `InventoryLog` write on top. `git diff HEAD -- src/lib/inventory.test.ts` is purely additive (0 removed `it(` blocks; new `describe("applyStockDelta ...")` block with 3 tests). Ran `inventory.test.ts` in isolation: 19/19 pass (16 pre-existing HUB-29 tests + 3 new). Confirmed zero regression, not just "pass count went up."
4. **Coupon-race trace**: `redeemCoupon()` (`src/lib/storefront/coupon.ts`) is a single guarded `UPDATE coupons SET used_count = used_count + 1 WHERE id = $id AND is_active = true AND (max_uses IS NULL OR used_count < max_uses) AND (expires_at IS NULL OR expires_at > now())`, throwing `CouponRedemptionRaceError` on 0 rows affected — the same conditional-UPDATE pattern as `adjustStock()`, applied to `usedCount` instead of `stockQuantity`. `placeOrder()` calls it inside its own transaction, catches `CouponRedemptionRaceError` at the top-level try/catch (outside the transaction callback) and maps it to `{ ok: false, error: "coupon_no_longer_valid" }`. Because the throw happens _inside_ the `db.$transaction(async (tx) => {...})` callback, Prisma rolls back the entire transaction on any throw — so an earlier-in-the-same-transaction stock decrement is correctly undone, not left as a partial write. Confirmed via the exact scenario the task specified: `checkout.test.ts`'s `"returns coupon_no_longer_valid when redemption races (0 rows affected)"` test sets `maxUses: 1, usedCount: 0` (passes the pre-transaction `evaluateCoupon()` check) then mocks `redeemCoupon` to reject with the race error (simulating a concurrent order having consumed the last use between read and write), and asserts `mockOrderCreate` was never called — i.e., no partial commit reaches the DB layer, and no 500 (clean typed error).
5. **Deterministic stock-decrement ordering**: read the actual code in `checkout.ts` — `lineSortKey()` returns `line.variantId ?? line.productId`, and `sortedLines = [...resolvedLines].sort(...)` by that key runs immediately before the `applyStockDelta` loop. This is real code, not just a comment claim. Confirmed further by the `"decrements stock lines sorted by variantId ?? productId ascending"` test, which seeds cart items in reverse order (p2 then p1) and asserts `applyStockDelta` was called for p1 before p2.
6. **Order immutability / empty-cart rejection**: `grep`'d `checkout.ts` for `order.update`/`upsert` — none exist; Order/OrderItem/InventoryLog are create-only. Empty-cart is checked twice: once before opening the transaction (`db.cart.findFirst` returns null → `cart_empty`) and again inside the transaction after re-fetching items (`items.length === 0` → `cart_empty`, covering the race where the cart is emptied between the pre-check and the transaction). Both paths have direct unit tests (`"returns cart_empty when the user has no cart at all"`, `"returns cart_empty when the cart has no items"`).
7. **New pure/unit test spot-check**: `tax.test.ts` (3 tests, confirms `calculateTax()` returns exactly `0` regardless of input — correctly matches product-planning's confirmed "no tax-rate decision exists yet" scope, with a doc comment explicitly forbidding a guessed nonzero rate); `coupon.test.ts`'s new `redeemCoupon` tests (2 tests, exercise both the 1-row-affected success path and the 0-row-affected race path against a mocked `tx`); `address.test.ts` (7 tests across `listAddresses`/`createAddress`/`getOwnedAddress`, explicitly covering the never-trust-client-supplied-userId/addressId ownership-isolation cases that match security-reviewer's noted informational Low about the duplicate ownership check in the transaction). All meaningful, not tautological.
8. **Scope leakage check**: `git diff HEAD --stat -- prisma/schema.prisma` is empty (untouched). No `order.update`, no `Payment` row creation (only a doc comment noting it's explicitly deferred to HUB-40), no gateway logic, no WhatsApp code, no order-management/tracking code anywhere in the new/modified checkout files.
9. **Dogfood**: `scripts/` still has no checkout-specific or unified dogfood entrypoint — same pre-existing gap as every prior ticket in this repo's history, reconfirmed, not a new blocker introduced by this ticket.

**Verdict: PASS.** All 9 verification points confirmed by direct code reading and independent test re-execution (not by trusting the builder's or security-reviewer's reports). No regressions, no scope leakage, live concurrency test is genuinely real and reproducibly green.

**Rule going forward — live concurrency test methodology worth reusing for future money/stock-critical tickets:** (a) always re-run the specific live-DB test file in isolation yourself, more than once, and check it's exercising real DB round-trip latency (multi-second duration, not millisecond mock-speed) as a tell that it isn't secretly mocked; (b) verify the test asserts on the _loser's_ side effects too, not just the winner's (here: losing cart untouched, no Order row for the loser) — a test that only checks "one success + one failure" can pass even if the failure path leaked a partial write; (c) when a refactor extracts a previously-reviewed atomic-guard SQL statement into a new function, diff the exact SQL text character-for-character rather than trusting a doc comment's "preserves identical guard" claim, and confirm the pre-existing test count for that function didn't shrink (count `it(` removals in the diff, not just the final pass count) — a builder could silently delete and re-add a similar-but-different test and the net count would look unchanged; (d) for cross-cutting atomic-guard patterns (stock via `adjustStock`/`applyStockDelta`, coupon via `redeemCoupon`), confirm the _same_ WHERE-clause-guard idiom (conditional UPDATE, 0-rows-affected as the race signal, typed error class, caught at the transaction boundary) is genuinely reused rather than re-implemented differently each time — consistency here is itself evidence of a deliberate, reviewed design rather than ad hoc code.

---

## HUB-39: Order Management coverage audit (2026-09-05)

### Genuine gaps found and closed: pure-logic timeline + public route-level tests

**Symptom:** `src/lib/order-timeline.ts` (the shared timeline-shaping logic used by both the authenticated order-detail page and the public tracking page) and `src/app/api/track/route.ts` (the one order-reading endpoint with NO auth, i.e. the highest-risk surface) both had 0% coverage — confirmed by running `npm run test:coverage` and reading the per-file report, not assumed. `src/lib/api/orders.ts` itself was NOT a gap: it has no mocked unit test file, only `orders.live.test.ts`, but that live test is genuinely real (re-ran it standalone against the actual dev Postgres, 9/9 pass, multi-hundred-ms-to-second durations proving real round trips) and already drives `orders.ts` to 95.65%/80%/100%/100% — a mocked-duplicate test would have added no real coverage or confidence, so it was correctly not added (avoids the padding anti-pattern this repo's audit history explicitly watches for).

**Cause:** `order-timeline.ts` is pure, framework-free logic (easy to unit test) but had simply never been given a test file. `track/route.ts` was the only API route under `src/app/api/**` in the whole repo with no `route.test.ts` — every sibling route (`address`, `cart`, `cart/merge`, `cart/price`, `checkout`, `coupons/validate`, `products`, `wishlist`, etc.) has one, so this was a real deviation from repo convention, not an intentional exception.

**Solution implemented:**

1. `src/lib/order-timeline.test.ts` (8 tests) — covers: in-progress order (mixed reached/pending stages), single-history-row order, fully DELIVERED order (all 4 stages reached), CANCELLED order (only stages actually reached + CANCELLED, SHIPPED/DELIVERED never implied), immediate-cancellation edge case, repeated-status history (keeps first timestamp), empty history (defensive PLACED prepend), and CANCELLED-with-no-PLACED-row edge case.
2. `src/app/api/track/route.test.ts` (9 tests) — mocks `trackOrder` (not the DB directly, matching this repo's per-route-test convention) but uses the REAL `rateLimiter` (cleared in `beforeEach`, same pattern as `coupons/validate/route.test.ts` and `checkout/route.test.ts`). Covers: 400 on malformed body / invalid JSON (with `trackOrder` never called), 200 with the exact reduced shape (explicitly asserts `shippingAddress`/`paymentMethod`/`email` are NOT present in the response — proving Iron Rule #6 reduced-shape at the HTTP boundary, not just in the lib function), identical generic 404 for two different failure reasons (bad suffix vs. wrong email) asserted via `toEqual` against each other, 500 with a generic message that does NOT leak the underlying error string, and — the specific thing the dispatching task flagged as unverified — **both rate-limit buckets independently reject, and in both cases `trackOrder` (i.e. the DB call) is never invoked**: one test drives the IP bucket down using a fixed IP + varying suffixes, the other drives the suffix bucket down using a fixed suffix + varying IPs (proving an attacker can't defeat the suffix-scoped limit by rotating IPs), plus a control test proving a normal within-limits request still reaches `trackOrder`.

**On the `orders.live.test.ts` "order with no linked user" scenario:** the dispatching task asked me to confirm this collapse case was tested. Checked `prisma/schema.prisma` — `Order.userId` is a required `String` field (`onDelete: Cascade` from `User`), so an order with no linked user is not a reachable state in this schema; the live test's existing "wrong suffix" and "wrong email" cases already exercise the two states that ARE reachable, and `trackOrder()`'s `candidates.find(...)` logic handles a null-email join generically (not a special-cased branch), so no additional test was needed here — this was a false-positive gap in the dispatching task's framing, correctly identified as such rather than padded with an untestable scenario.

**Rule going forward:** when auditing an existing feature's test coverage, always run `npm run test:coverage` and read the per-file table before trusting a builder's self-reported test list — `orders.live.test.ts` existing and being real does NOT mean every file the feature touches has coverage; `order-timeline.ts` and `track/route.ts` both sat at a literal 0% despite the feature being "done" and passing security review. Cross-check new API routes against the full `find src/app/api -iname "*.test.ts"` list to catch convention deviations (missing `route.test.ts`) immediately. Before accepting a task's framing of "is X tested?", check whether X is even a reachable code path against the actual Prisma schema (required vs. optional FK) rather than writing a test for an impossible state.

### Dogfood entrypoint: pre-existing gap for the full checkout→order path, PLUS new local DB-connectivity finding

**Symptom:** No unified "browse → cart → checkout → payment → order" dogfood entrypoint exists anywhere in the repo — `scripts/` only has `dogfood-{u3,u4,u5,hur51}.ts`, each scoped to its own early milestone (auth, i18n, catalog, CI/CD baseline). This is NOT something HUB-39 introduced; it's a standing gap across the whole M5 (checkout/orders/payments) surface, already flagged in a prior `checkout.live.test.ts` audit entry above ("Dogfood: `scripts/` still has no checkout-specific or unified dogfood entrypoint"). Building the full version would require orchestrating real payment-rail flows, which is blocked by documented open risks (eDahab has no sandbox; WaafiPay base URL unconfirmed — `docs/agents/run-state.md` Active Decisions #6-7) and is cross-cutting with payment-gateways' ownership, not something to improvise solo here.

**What was added instead:** `scripts/dogfood-hub39.ts` — scoped strictly to HUB-39's own new HTTP surface (matches this repo's existing per-unit dogfood pattern, e.g. `dogfood-u5.ts`): starts `next dev`, waits on `/api/health`, then verifies (1) unauthenticated `GET /en/account/orders` redirects to signin, (2) unauthenticated `GET /en/account/orders/<id>` gets the same protection, (3) public `GET /en/track` renders without auth, (4) `POST /api/track` 400s on a malformed body, (5) `POST /api/track` returns the generic 404 collapse for a non-matching suffix/email. This is real and exit-code-clean when the DB is reachable (verified — see below).
