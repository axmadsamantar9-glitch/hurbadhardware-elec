...
## HUB-40: Payment System -- Structurally Verified, Live Credentials Pending (2026-09-06)

**Symptom:** First dogfood-hub40.ts run failed: two checks returned "fetch failed" and a third check hung indefinitely (over 10 minutes, only flushed to the output log once the process was force-killed).

**Cause:** Not a code defect. The script self-spawns its own dev server via child_process.spawn("cmd", ["/c", "npm run dev"]). In this session a stale dev-server process from an earlier interrupted run was still bound to port 3000, so the freshly-spawned server either could not bind cleanly or the fetches raced against a half-initialized process. On Windows, stdout piped through cmd /c is not flushed to the harness output file until the child process exits, which made the hang look silent/unexplainable until forcibly killed.

**Fix applied:** Killed all stray node processes bound to port 3000 (confirm via `Get-NetTCPConnection -LocalPort 3000`), then re-ran the dogfood script cleanly -- 9/9 passed, exit 0. Independently corroborated every one of the 9 endpoint behaviors by hand with direct node fetch() calls (with AbortController timeouts) against a manually-started dev server before trusting the clean re-run, since the first run's ambiguous failure mode (silent hang) was worth independently reproducing rather than just re-running and accepting a pass.

**Rule going forward:** Before running any dogfood script that self-spawns a dev server, check `Get-NetTCPConnection -LocalPort 3000` (or `netstat -ano | grep :3000`) for a stray listener and kill it first. If a dogfood run produces an ambiguous result (hang, empty output, "fetch failed" with no further detail), do not simply mark it RED or blindly re-run and accept a green -- independently probe the specific endpoints by hand (direct fetch with a timeout) to distinguish "real code defect" from "port/process contention artifact of the harness" before making a final call either way.
