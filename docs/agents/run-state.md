...
4. **RESOLVED (2026-08-23, HUR-51): CI/CD pipeline now exists** (`.github/workflows/ci.yml` — typecheck, lint, test, build on every push/PR to `main`). This entry was stale; corrected during the 2026-09-05 FEATURES.md reconciliation pass.
5. **RESOLVED (2026-09-05): Production database authentication blocker.** User corrected the Vercel Production `DATABASE_URL` to the correct Supabase Transaction Pooler connection string (with the proper `postgres.<project-ref>` username). Confirmed live: `/api/health` → `{"status":"ok","database":"ok"}`, `/en` and `/so` → HTTP 200. This unblocks new feature dispatch as far as database connectivity is concerned — see items 6 and 7 below for two newly-discovered, unrelated blockers found during the post-fix verification pass.
6. **RESOLVED (2026-09-05): Production auth (`AUTH_SECRET`) blocker.** User set `AUTH_SECRET` in Vercel Production and redeployed. Confirmed live: `GET /api/auth/session` returns clean `200` (was `500` "server configuration" error); a real sign-in with the seeded admin account succeeded and produced a valid session (`role: ADMIN`, correct 30-day expiry); `/en/checkout` now renders the full checkout flow instead of the `checkout.signInRequired` gate. Checkout is no longer blocked by authentication.
7. **RESOLVED (2026-09-05): Client-side i18n sitewide bug.** `src/app/providers.tsx` rendered `<NextIntlClientProvider locale={locale}>` with no `messages` prop, so every `"use client"` component's translations silently failed and rendered the raw dotted key. Fixed: `src/app/[locale]/layout.tsx` now calls `getMessages({ locale })` and forwards it through `Providers` into `NextIntlClientProvider` (commit `d4e5e18`). Verified locally (production build+start) and confirmed live in production post-redeploy: no raw i18n keys on `/en/auth/signin`, `/en/cart`, or product detail pages.
8. **RESOLVED (2026-09-05): Vercel build fails when only `prisma/schema.prisma` changes.** HUB-39's first deploy (`074fd5c`) failed with dozens of "property does not exist on PrismaClient" typecheck errors for the brand-new `OrderStatusHistory` model/`Order.trackingNumber` field. Root cause, confirmed from the actual Vercel build log: Vercel restored a cached `node_modules` from the prior deployment and logged `npm install` as "up to date" since neither `package.json` nor the lockfile had changed — so `@prisma/client`'s postinstall-triggered `prisma generate` never re-ran, and the build typechecked against the stale pre-HUB-39 generated client. Fixed (`313962b`) by changing the `build` script to `prisma generate && next build`, forcing regeneration on every build regardless of install-cache state. Reproduced locally first (deleting `node_modules/.prisma` and running the old `next build` alone reproduces the identical error class) before shipping the fix, and confirmed the redeploy succeeded and HUB-39 works end-to-end in production. **This was a latent risk since the very first migration in this repo** — any prior deploy that touched only `schema.prisma` without a `package.json`/lockfile change would have hit this; it just happened to be triggered first by HUB-39.
9. **OPEN (2026-09-05): Category and product images are all 404 in production.** Seed data (`prisma/seed.ts`) writes local-relative image URLs (`/images/categories/<slug>.jpg`, `/images/products/<slug>-N.jpg`) that were never backed by real files — `public/images/` does not exist in the repository at all. `src/lib/image-loader.ts` correctly passes these through unchanged (by design, for local/relative assets), so this is not a loader bug. Two possible fixes, neither purely code: (a) add real product/category photography to `public/images/...` and commit it, or (b) wire up the already-scaffolded Cloudflare Images/R2 pipeline (`NEXT_PUBLIC_CLOUDFLARE_IMAGES_ACCOUNT_HASH` + R2 credentials, referenced in HUB-28's notes as unavailable in this environment) and re-seed with real CDN URLs. Neither is a quick repository fix; needs a product decision on which path plus actual image assets or CDN credentials.
10. **OPEN (2026-09-09, found by production-readiness-gate while gate-verifying HUB-43): `/api/admin/*` API routes are intercepted by the page-level auth-redirect middleware, not just their own `requireAdmin()` handler logic.** `src/proxy.ts`'s sitewide protected-route check is `pathname.includes("/account") || pathname.includes("/admin")`, which matches API paths like `/api/admin/rma`, `/api/admin/warranties`, `/api/admin/payments` (not just page paths like `/admin/rma`). Consequence: for a normal browser `fetch()` (default `redirect: "follow"`), an unauthenticated or non-admin request to any `/api/admin/*` route never reaches the route handler's own `requireAdmin()` check -- it gets a 307 redirect to `/en/auth/signin?callbackUrl=...` at the edge first, and the client-side code transparently follows it and lands on signin HTML instead of the route's intended 401/403 JSON response. The route handlers' own 401/403 JSON logic is real and test-covered, but only observable by a caller using `redirect: "manual"` (confirmed via direct `curl`/`fetch(..., {redirect: "manual"})}` testing, not just theorized). First surfaced during HUB-42's dogfood run (2026-09-09) and re-confirmed as still present, identical behavior, during HUB-43's independent gate-verification (2026-09-09) -- affects every `/api/admin/*` route shipped since HUB-40 (payments, warranties, RMA), not specific to any one item. Not a security hole (the auth boundary IS enforced, just earlier and via redirect rather than JSON), but it is a footgun for any API consumer expecting a JSON 401/403 (e.g. a future mobile app or third-party integration hitting these routes directly) and it silently breaks a dogfood-style script's naive `res.status === 401` assertion. **Needs an architecture decision from the orchestrator/architect:** should `src/proxy.ts` exclude `/api/*` paths from its page-redirect branch (relying solely on each route handler's own `requireAdmin()`/`requireAuth()` JSON response), or is the redirect-first behavior intentional and future API consumers should be documented to use `redirect: "manual"`? Not fixed here -- cross-cutting change affecting all admin API routes, out of scope for any single feature item to silently alter.
