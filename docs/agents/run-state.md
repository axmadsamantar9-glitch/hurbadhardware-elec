...
5. **OPEN (2026-09-05): Production database authentication blocker.** `hurbadhardware-elec.vercel.app/en` returns HTTP 500; `/api/health` reports `database: unreachable`. Root cause: Prisma authenticates to Supabase as plain `postgres` instead of the required Supavisor-pooler username `postgres.jhfhhmhomvxkiwmzantc` (confirmed via the live error text: "Authentication failed... credentials for `postgres` are not valid"). Repository-side causes are conclusively ruled out (single `PrismaClient`, single `datasource` reading `env("DATABASE_URL")` unmodified, no code/config override anywhere, no committed `.env*`). This is a Vercel-side `DATABASE_URL` environment-variable value/scope issue requiring manual dashboard action (see chat history for exact steps: check for a duplicate/mis-scoped `DATABASE_URL` row across Production/Preview/Development, or re-copy the connection string verbatim from Supabase's Connect → Transaction pooler tab). **Blocks all new feature dispatch** — the production-readiness-gate's dogfood step cannot pass against a database-unreachable deployment, and per explicit user instruction, no new feature work should start until this is resolved and `/en` is verified rendering live.
