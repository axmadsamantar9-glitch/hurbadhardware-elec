...
6. **OPEN (2026-09-05): Production auth is broken — `AUTH_SECRET` likely missing/invalid in Vercel.** `GET /api/auth/session` returns HTTP 500, `"There was a problem with the server configuration"` — NextAuth's generic error for a missing/invalid `AUTH_SECRET` (or comparable provider misconfiguration) in production. Effect: the sign-in page renders, but no session can ever be established — **checkout is fully blocked** (its `/en/checkout` route hard-requires a session: `checkout.signInRequired`, and guest checkout is not implemented). `src/auth.ts` was inspected and contains no code-level bug (`trustHost: true` set, Google provider correctly gated behind its own env vars, Prisma adapter reused from the single `db` singleton). This is a Vercel Production environment-variable gap requiring manual action: set `AUTH_SECRET` in Vercel → Settings → Environment Variables → Production (generate one locally with `npx auth secret` if the original value is lost), then redeploy.
7. **OPEN (2026-09-05): Client-side i18n is broken sitewide — `NextIntlClientProvider` never receives `messages`.** `src/app/providers.tsx` renders `<NextIntlClientProvider locale={locale}>` with no `messages` prop, so every `"use client"` component's `useTranslations()`/`t()` calls resolve to nothing and render the raw dotted key instead of text. Confirmed live on the sign-in form (`auth.signin`, `auth.email`, etc.), the product-detail add-to-cart button (`product.addToCart`), and the entire cart page (`cart.title`, `cart.subtotal`, `cart.checkout`, ...) — all literal keys visible in the rendered page. Server-rendered text (homepage hero, category names, nav) is unaffected since server components get messages directly from `src/i18n.ts`'s request config, bypassing this provider entirely. The underlying cart/coupon/pricing logic itself is unaffected and verified correct (WELCOME10 coupon applied a correct 10% discount live) — this is purely a missing-translations display bug, but it affects nearly every interactive surface on the site. **This is a repository-side bug with a known, contained fix**: fetch messages server-side (`getMessages()` from `next-intl/server`) in `src/app/[locale]/layout.tsx` and pass them into `<Providers locale={locale} messages={messages}>` → `<NextIntlClientProvider locale={locale} messages={messages}>`. Not yet fixed — awaiting go-ahead.
8. **OPEN (2026-09-05): Category and product images are all 404 in production.** Seed data (`prisma/seed.ts`) writes local-relative image URLs (`/images/categories/<slug>.jpg`, `/images/products/<slug>-N.jpg`) that were never backed by real files — `public/images/` does not exist in the repository at all. `src/lib/image-loader.ts` correctly passes these through unchanged (by design, for local/relative assets), so this is not a loader bug. Two possible fixes, neither purely code: (a) add real product/category photography to `public/images/...` and commit it, or (b) wire up the already-scaffolded Cloudflare Images/R2 pipeline (`NEXT_PUBLIC_CLOUDFLARE_IMAGES_ACCOUNT_HASH` + R2 credentials, referenced in HUB-28's notes as unavailable in this environment) and re-seed with real CDN URLs. Neither is a quick repository fix; needs a product decision on which path plus actual image assets or CDN credentials.
