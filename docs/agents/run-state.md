...
7. **FIXED, pending live re-verification (2026-09-05): Client-side i18n sitewide bug.** `src/app/providers.tsx` rendered `<NextIntlClientProvider locale={locale}>` with no `messages` prop, so every `"use client"` component's translations silently failed and rendered the raw dotted key. Fixed: `src/app/[locale]/layout.tsx` now calls `getMessages({ locale })` and forwards it through `Providers` into `NextIntlClientProvider` (commit `d4e5e18`). Verified against a local production build+start (no raw keys in `/en/auth/signin` or `/en/cart` HTML); live-production confirmation after redeploy tracked in chat history.
