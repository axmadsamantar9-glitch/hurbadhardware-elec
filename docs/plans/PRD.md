---
title: HurbadHardware East Africa Electronics Commerce Platform — Complete Production Plan v4
type: product-requirements-and-implementation-plan
date: 2026-08-17
...
- purchase through locally relevant payment methods;
- track orders;
- request cancellation/returns/refunds/support;
...
- manage promotions, content and campaigns;
- operate customer support;
- audit sensitive actions;
- reconcile financial/payment records;
...
8. Webhooks are authenticated where supported, deduplicated and idempotent.
9. Business rules are never guessed.
...
1. **Merchant/payment accounts**
   - Confirm production merchant accounts for every v1 payment rail.
   - Confirm account ownership, KYC/onboarding status and settlement configuration.
2. **Payment production configuration**
   - Confirm production API endpoints, webhook endpoints, credentials, signing/verification requirements and callback behavior for each enabled provider.
   - Confirm which payment currencies each rail accepts and settles.
3. **Shipping**
   - Confirm launch countries, delivery zones, shipping methods, shipping prices and any free-shipping thresholds.
   - Confirm expected delivery times by zone.
4. **Tax**
   - Confirm applicable tax treatment and rates by launch market/zone.
   - Confirm whether prices are tax-inclusive or tax-exclusive.
5. **Returns/refunds**
   - Confirm return window, eligible products/conditions, refund rules, partial-refund rules and who pays return shipping.
6. **Warranty**
   - Confirm warranty duration, coverage, exclusions, claim process and whether manufacturer/supplier warranty differs by product.
7. **Launch catalog**
   - Confirm the actual launch products, SKUs, prices, product specifications, media, compatibility information, condition and warranty data.
8. **Supplier data**
   - Confirm supplier identities, procurement information and required receiving/inventory data.
9. **Delivery coverage**
   - Confirm exact cities/regions/zones served at launch and any areas excluded from delivery.
10. **Legal and policy content**
    - Approve privacy, terms, shipping, returns/refunds, warranty, cookie and support policy wording.
11. **WhatsApp Business**
    - Confirm the production WhatsApp Business account, phone number, Cloud API access, webhook configuration and approved message/template requirements.
12. **Production infrastructure**
    - Confirm the production deployment/account ownership, database, domain, DNS, email delivery service and backup ownership.
13. **Operational ownership**
    - Assign owners for payments/reconciliation, inventory, fulfillment, support, security incidents and disaster recovery.
14. **Launch approval**
    - Name the business owner authorized to make the final GO/NO-GO decision.

## Gate behavior

- Missing decisions block only the affected implementation area where possible.
- No production credential may be placed in source control, client code or logs.
- No payment integration is considered production-ready until real merchant configuration and end-to-end staging verification are complete.
- No catalog item is considered launch-ready until it passes the product completeness gate.
- No launch date is considered committed until all critical business decisions and production dependencies are confirmed.
...
- Loyalty points
- Flash-sale engine
- Direct Daraja replacement of Paystack unless justified by volume
...
- authenticity policy;
- open-box/refurbished policy;
- product specifications;
- supplier information;
- payment merchant configuration;
- gateway production endpoints;
- legal policy wording;
- support hours;
- data retention;
- marketing consent;
- actual launch catalog.

If one of these is missing, stop only the affected implementation area and surface the decision.
...
- Audit logs.
- Payment/webhook/notification events.
- Indexed query paths and connection pooling.
...
- Managed PostgreSQL.
- Scheduled jobs for FX, reconciliation, notifications and maintenance.
...
- Warranty terms, exclusions and coverage notes are snapshotted against the applicable order/product record so historical warranty decisions remain auditable.
- Warranty may differ by product condition (NEW / OPEN_BOX / REFURBISHED / USED) where the business policy allows it.
...
- Serial number / IMEI / MAC / batch identifier when applicable.
- Purchase and delivery dates.
- Warranty start and expiry dates.
- Warranty provider/responsible party.
- Coverage type and terms.
- Current status.
- Claim history.
- Attachments such as invoice, photos, diagnostic evidence and service documents.
...
- Customers must be able to view eligible active warranties from their account.
...
- Diagnostic findings.
- Parts/service used where applicable.
- Repair cost and responsibility.
- Replacement product/serial where applicable.
- Refund amount where applicable.
- Claim SLA/target date.
...
- A replacement product must receive its own warranty record according to the approved replacement-warranty policy.
- Warranty actions must not silently modify historical order data.
...
4. E2E: complete customer/admin journeys.
5. Security: dependency, secret, IDOR, injection, XSS, CSRF, upload, rate-limit and webhook tests.
6. Performance: Core Web Vitals and low-bandwidth tests.
...
- Failed/expired payment → inventory restoration.
- Payment race → exactly one terminal success.
- Coupon concurrency.
- Out-of-stock atomic failure.
- Product/variant/brand creation.
- Inventory adjustment/stocktake.
- Cancellation.
- Return/RMA/refund/partial refund.
- Warranty registration and eligibility.
- Warranty claim approval/rejection.
- Warranty repair/replacement/refund resolution.
- Warranty expiry and authorized override.
- WhatsApp order.
- Notifications.
- Search/filter/brand/category.
- EN/SO.
- Mobile checkout.
...
4. Historical orders never change when products change.
5. Admin authorization is server-side.
6. Sensitive data is not exposed publicly.
7. Third-party integrations are isolated behind adapters.
8. Webhooks are idempotent.
9. Business rules are not guessed.
10. Production deployment requires passing automated and manual critical gates.

## 53. Master Build Order

The implementing agent should execute in this order:

1. Foundation, architecture and environment separation.
2. Database and immutable core entities.
3. Authentication, authorization and security foundation.
4. Brands, categories and product master data.
5. Product specifications, variants, media and compatibility.
6. Catalog, search, discovery and SEO foundation.
7. Cart, wishlist and comparison.
8. Pricing, promotions, coupons, tax and shipping engine.
9. Inventory ledger and reservation engine.
10. Checkout and order state machine.
11. Payment adapters and reconciliation.
12. Customer accounts and tracking.
13. Notifications and WhatsApp.
14. Returns, refunds, warranty and RMA.
15. Admin operations, support and content.
16. Procurement/supplier and receiving foundations.
17. Analytics, marketing and reporting.
18. Accessibility, performance and low-bandwidth optimization.
19. Security testing, observability and disaster recovery.
20. Full staging E2E, business acceptance and production launch.
...
- [ ] Launch countries and delivery coverage confirmed.
- [ ] Shipping zones, prices and delivery times confirmed.
- [ ] Tax treatment and rates confirmed.
- [ ] Return/refund policy confirmed.
- [ ] Warranty rules confirmed.
- [ ] Warranty claim workflow, responsible party and replacement-warranty policy confirmed.
- [ ] Launch catalog and required product data confirmed.
- [ ] Supplier information confirmed.
- [ ] Production payment merchant accounts confirmed.
- [ ] Payment production endpoints, credentials and webhook requirements confirmed.
- [ ] WhatsApp Business production configuration confirmed.
- [ ] Legal/policy content approved.
- [ ] Production infrastructure ownership and domains confirmed.
- [ ] Operational owners assigned.
- [ ] Backup/recovery ownership and restore responsibility assigned.

If a checkbox is not satisfied, the agent must not fabricate the missing business value. It should continue with unaffected technical work and surface the missing decision for confirmation.
...
- Payment gateway production endpoint/credential.
- Merchant account configuration.
- Legal policy wording.
- Country availability.
- Supported delivery areas.

Everything else that is purely technical may be implemented using the architecture and invariants in this plan.
...
- Order totals immutable.
- Cancellation/returns/refunds/RMA tested.
- Warranty registration, eligibility, claim, repair/replacement/refund and expiry flows tested.
...
## 13. Canonical Build Rules

1. Follow the Master Build Order.
2. Do not skip backend integrity because UI is complete.
3. Do not create provider-specific business logic outside adapters.
4. Do not guess business values.
5. Every new feature must preserve the permanent invariants.
6. Every schema change requires a migration.
7. Every critical requirement needs automated or documented manual verification.
8. Every production change needs rollback consideration.
...
- A dedicated **Implementation Agent Handoff Checklist**.
- Explicit separation between technical work that can proceed and business decisions that must be confirmed.
...
- Warranty claim, diagnosis, repair, replacement and refund workflow.
- Authorized warranty overrides with audit trails.
- Replacement-product warranty handling.
...
- Existing deferred-v2 scope.
- Existing business decision register.

# Appendix B — Consolidated Plan Contents

This file is the single consolidated HurbadHardware production plan. It includes the original PRD together with the pre-build readiness additions and full warranty-management requirements.

The consolidated plan covers:

- Product scope and market strategy
- Business decision register
- Pre-build readiness gate
- Product/catalog and electronics-specific data
- Pricing, promotions, tax and shipping
- Inventory and reservations
- Checkout and order lifecycle
- Payment integrations and reconciliation
- Customer accounts and order tracking
- WhatsApp ordering and notifications
- Warranty management
- Returns, refunds and RMA
- Admin operations and support
- Procurement and supplier management
- Marketing, SEO and analytics
- Security, privacy and reliability
- Observability, backups and disaster recovery
- QA and critical E2E validation
- Master implementation roadmap
- Implementation-agent handoff checklist
- Final NO-GUESS gate
- Production Definition of Done and GO/NO-GO launch criteria
- Post-launch monitoring and controlled optimization
