...
describe('getProducts', () => {
  ...
  describe('Pagination', () => {
    ...
    it('handles page beyond available results', async () => {
      ...
      vi.mocked(db.product.findMany).mockResolvedValue([])
      const result = await getProducts({ page: 100, limit: 20, search: '', category: '', brand: '' })
  ...
  describe('Full-Text Search', () => {
    it('searches products via tsvector', async () => {
      ...
      vi.mocked(db.product.findMany).mockResolvedValue([mockProduct])
      const result = await getProducts({ page: 1, limit: 20, search: 'Samsung', category: '', brand: '' })
    ...
    it('returns empty results for no matches', async () => {
      ...
      const result = await getProducts({ page: 1, limit: 20, search: 'nonexistent', category: '', brand: '' })
  ...
  describe('Brand Filter', () => {
    it('filters by brand case-insensitive', async () => {
      ...
      expect(db.product.count).toHaveBeenCalledWith(
        expect.objectContaining({ where: expect.objectContaining({ brand: { contains: 'samsung', mode: 'insensitive' } }) })
      )
    })
  })
  ...
  describe('Price Range Filter', () => {
    it('filters by min price', async () => {
      ...
      expect(db.product.count).toHaveBeenCalledWith(
        expect.objectContaining({ where: expect.objectContaining({ basePriceUsd: { gte: new Decimal('100') } }) })
      )
    })
    ...
    it('filters by max price', async () => {
      ...
      expect(db.product.count).toHaveBeenCalledWith(
        expect.objectContaining({ where: expect.objectContaining({ basePriceUsd: { lte: new Decimal('500') } }) })
      )
    })
    ...
    it('filters by both min and max price', async () => {
      ...
      vi.mocked(db.product.findMany).mockResolvedValue([mockProduct])
      await getProducts({ page: 1, limit: 20, search: '', category: '', brand: '', priceMin: 200, priceMax: 500 })
      expect(db.product.count).toHaveBeenCalledWith(
        expect.objectContaining({ where: expect.objectContaining({ basePriceUsd: { gte: new Decimal('200'), lte: new Decimal('500') } }) })
      )
    })
  })
  ...
  describe('Filter Combinations', () => {
    it('combines search + category', async () => {
      ...
      vi.mocked(db.product.count).mockResolvedValue(1)
      vi.mocked(db.product.findMany).mockResolvedValue([mockProduct])
      await getProducts({ page: 1, limit: 20, search: 'Samsung', category: 'smartphones', brand: '' })
      expect(db.$queryRaw).toHaveBeenCalled()
    })
    ...
    it('applies all filters together', async () => {
      ...
      vi.mocked(db.product.count).mockResolvedValue(1)
      vi.mocked(db.product.findMany).mockResolvedValue([mockProduct])
      await getProducts({ page: 1, limit: 20, search: 'phone', category: 'smartphones', brand: 'Samsung', priceMin: 100, priceMax: 500 })
      expect(db.product.count).toHaveBeenCalledWith(
  ...
  describe('Input Validation', () => {
    it('throws when limit exceeds 100', async () => {
      await expect(getProducts({ page: 1, limit: 101, search: '', category: '', brand: '' })).rejects.toThrow('Limit exceeds maximum')
    })
  ...
  describe('SQL Injection Protection', () => {
    it('prevents SQL injection in category', async () => {
      ...
      vi.mocked(db.product.findMany).mockResolvedValue([])
      const result = await getProducts({ page: 1, limit: 20, search: '', category: "test' OR '1'='1", brand: '' })
      expect(result.total).toBe(0)
    })
    ...
    it('prevents SQL injection in brand', async () => {
      vi.mocked(db.product.count).mockResolvedValue(0)
      vi.mocked(db.product.findMany).mockResolvedValue([])
      const result = await getProducts({ page: 1, limit: 20, search: '', category: '', brand: "'; DROP TABLE; --" })
      expect(result.total).toBe(0)
    })

    it('prevents SQL injection in search', async () => {
      vi.mocked(db.$queryRaw).mockResolvedValue([])
      const result = await getProducts({ page: 1, limit: 20, search: "test'; UNION SELECT; --", category: '', brand: '' })
      expect(result.total).toBe(0)
    })
  })
